...
func (c *crtbLifecycle) reconcileSubject(binding *v3.ClusterRoleTemplateBinding) (*v3.ClusterRoleTemplateBinding, error) {
	...
	if binding.GroupName != "" || binding.GroupPrincipalName != "" || (binding.UserPrincipalName != "" && binding.UserName != "") {
		addCondition(binding, condition, subjectExists, nil)
		return binding, nil
	}
	...
	if binding.UserPrincipalName != "" && binding.UserName == "" {
		...
		if err != nil {
			addCondition(binding, condition, failedToCreateUser, err)
			return binding, err
		}
	...
	if binding.UserPrincipalName == "" && binding.UserName != "" {
		...
		if err != nil {
			addCondition(binding, condition, failedToGetUser, err)
			return binding, err
		}
	...
	addCondition(binding, condition, crtbHasNoSubject, fmt.Errorf("CRTB has no subject"))
...
func (c *crtbLifecycle) reconcileBindings(binding *v3.ClusterRoleTemplateBinding) error {
	...
	if binding.UserName == "" && binding.GroupPrincipalName == "" && binding.GroupName == "" {
		addCondition(binding, condition, bindingExists, nil)
		return nil
	}
	...
	if err != nil {
		addCondition(binding, condition, failedToGetCluster, err)
		return err
	}
	if cluster == nil {
		err = fmt.Errorf("cannot create binding because cluster %v was not found", clusterName)
		addCondition(binding, condition, clusterNotFound, err)
		return err
	}
	...
	if err != nil {
		addCondition(binding, condition, failedToCheckReferencedRole, err)
		return err
	}
	...
	if err != nil {
		addCondition(binding, condition, failedToBuildSubject, err)
		return err
	}
	if err := c.mgr.ensureClusterMembershipBinding(clusterRoleName, pkgrbac.GetRTBLabel(binding.ObjectMeta), cluster, isOwnerRole, subject); err != nil {
		addCondition(binding, condition, failedToEnsureClusterMembershipBinding, err)
		return err
	}

	err = c.mgr.grantManagementPlanePrivileges(binding.RoleTemplateName, clusterManagementPlaneResources, subject, binding)
	if err != nil {
		addCondition(binding, condition, failedToGrantManagementPlanePrivileges, err)
		return err
	}

	projects, err := c.projectLister.List(binding.Namespace, labels.Everything())
	if err != nil {
		addCondition(binding, condition, failedToListProjects, err)
		return err
	}
	for _, p := range projects {
		...
		if err := c.mgr.grantManagementClusterScopedPrivilegesInProjectNamespace(binding.RoleTemplateName, p.Name, projectManagementPlaneResources, subject, binding); err != nil {
			addCondition(binding, condition, failedToGrantManagementClusterScopedPrivilegesInProjectNamespace, err)
			return err
		}
	}
	addCondition(binding, condition, bindingExists, nil)
...
func (c *crtbLifecycle) reconcileLabels(binding *v3.ClusterRoleTemplateBinding) error {
	...
	*/
	...
	if binding.Labels[RtbCrbRbLabelsUpdated] == "true" {
		addCondition(binding, condition, labelsReconciled, nil)
		return nil
	}
	...
	if err != nil {
		addCondition(binding, condition, failedToGetRequirements, err)
		return err
	}
	...
	if err != nil {
		addCondition(binding, condition, failedToGetClusterRoleBindings, err)
		return err
	}
	bindingKey := pkgrbac.GetRTBLabel(binding.ObjectMeta)
	for _, crb := range crbs {
		...
		if retryErr != nil {
			addCondition(binding, condition, failedToUpdateClusterRoleBindings, err)
		}
	...
	if err != nil {
		addCondition(binding, condition, failedToListRB, err)
		return err
	}
	...
	if retryErr != nil {
		addCondition(binding, condition, failedToUpdateClusterRoleTemplateBindings, err)
	}
	return retryErr
}
...
func (c *crtbLifecycle) setCRTBAsCompleted(crtb *v3.ClusterRoleTemplateBinding) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		if reflect.DeepEqual(crtbFromCluster.Status.LocalConditions, crtb.Status.LocalConditions) {
			fmt.Println("LOCAL not updating")
			return nil
		}
		fmt.Println("LOCAL updating!!!!")
...
func addCondition(binding *v3.ClusterRoleTemplateBinding, condition metav1.Condition, reason string, err error) {
	...
	for i := range binding.Status.LocalConditions {
		...
		if condition.Type == localCondition.Type {
			...
			found = true
			fmt.Println("found " + localCondition.Type)
		}
	}
	if !found {
