...
func (grb *globalRoleBindingLifecycle) reconcileClusterPermissions(globalRoleBinding *v3.GlobalRoleBinding) error {
	...
	if err != nil {
		grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToGetGlobalRole, err)
	...
	if err != nil {
		grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToCluster, err)
	...
	if missedClusters {
		...
		grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToReconcileCRTBs, err)
		return err
	}
	return nil
}
...
func (grb *globalRoleBindingLifecycle) reconcileGlobalRoleBinding(globalRoleBinding *v3.GlobalRoleBinding) error {
	...
	if globalRoleBinding.GlobalRoleName == rbac.GlobalRestrictedAdmin {
		if err := grb.syncDownstreamClusterPermissions(subject, globalRoleBinding); err != nil {
			grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToSyncDownstreamClusterPermissions, err)
			return err
		}
	}
	...
	if crb != nil {
		...
		if updateSubject || updateRoleRef {
			...
			if _, err := grb.crbClient.Update(crb); err != nil {
				grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToUpdateClusterRoleBinding, err)
	...
	if err != nil {
		grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToCreateClusterRoleBinding, err)
		return err
	}
	...
	return grb.addRulesForTemplateAndTemplateVersions(globalRoleBinding, subject, condition)
}

func (grb *globalRoleBindingLifecycle) addRulesForTemplateAndTemplateVersions(globalRoleBinding *v3.GlobalRoleBinding, subject v1.Subject, condition metav1.Condition) error {
	...
	if err != nil && !apierrors.IsNotFound(err) {
		grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToGetGlobalRoleBinding, err)
		return err
	}
	isCatalogRole := false
	if gr != nil {
	...
	if isCatalogRole {
		...
		if err != nil {
			grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToGetRole, err)
			return err
		}
		...
		if err != nil {
			if apierrors.IsNotFound(err) {
				...
				if err != nil && !apierrors.IsAlreadyExists(err) {
					grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToCreateRoleBinding, err)
					return err
				}
			} else {
				grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, failedToGetRoleBinding, err)
				return err
			}
		}
	}

	grb.s.AddCondition(&globalRoleBinding.Status.LocalConditions, condition, globalRoleBindingReconciled, nil)
	return nil
}
...
func (c *globalRoleBindingLifecycle) updateStatus(grb *apisv3.GlobalRoleBinding) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		grbFromCluster, err := c.grbLister.Get(grb.Name)
		if err != nil {
			return err
		}
		if len(grb.Status.RemoteConditions) > 0 && status.CompareConditions(grbFromCluster.Status.RemoteConditions, grb.Status.RemoteConditions) {
			return nil
		}
		if len(grb.Status.RemoteConditions) == 0 && grbFromCluster.Status.SummaryRemote == status.SummaryCompleted {
			return nil
		}
