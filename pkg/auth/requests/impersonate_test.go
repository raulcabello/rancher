	...
	tests := map[string]struct {
		...
		"impersonate group": {
			...
			sarMock: func(req *http.Request) sar.SubjectAccessReview {
				mock := mocks.NewMockSubjectAccessReview(ctrl)
				mock.EXPECT().UserCanImpersonateUser(req, "user", "impUser").Return(true, nil)
				mock.EXPECT().UserCanImpersonateGroups(req, "user", []string{"impGroup"}).Return(true, nil)
		...
		"impersonate group not allowed": {
			...
			sarMock: func(req *http.Request) sar.SubjectAccessReview {
				mock := mocks.NewMockSubjectAccessReview(ctrl)
				mock.EXPECT().UserCanImpersonateUser(req, "user", "impUser").Return(true, nil)
				mock.EXPECT().UserCanImpersonateGroups(req, "user", []string{"impGroup"}).Return(false, nil)
		...
		"impersonate group error": {
			...
			sarMock: func(req *http.Request) sar.SubjectAccessReview {
				mock := mocks.NewMockSubjectAccessReview(ctrl)
				mock.EXPECT().UserCanImpersonateUser(req, "user", "impUser").Return(true, nil)
				mock.EXPECT().UserCanImpersonateGroups(req, "user", []string{"impGroup"}).Return(false, errors.New("unexpected error"))
