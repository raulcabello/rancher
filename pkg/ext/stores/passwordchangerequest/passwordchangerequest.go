...
func New(wranglerContext *wrangler.Context, authorizer authorizer.Authorizer) *Store {
	store := Store{
		secretClient: wranglerContext.Core.Secret(),
		secretCache:  wranglerContext.Core.Secret().Cache(),
...
func (s *Store) Create(
	...
	if userInfo.GetName() != objPasswordChangeRequest.Name {
	...
	pwdManager := password.NewManager(s.secretCache, s.secretClient)
	if err := pwdManager.UpdateSecret(objPasswordChangeRequest.Spec.UserID, objPasswordChangeRequest.Spec.CurrentPassword, objPasswordChangeRequest.Spec.NewPassword); err != nil {
		c := metav1.Condition{
